/* Astro Night Planner 1.0.3 – Produktivversion mit korrigiertem Erststart und LDN-Katalog */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'prod', appVersion:'1.0.3', release:'1.0.3', databaseName:'astro-night-planner-prod-v1', documentTitle:'Astro Night Planner 1.0.3'});
...
const DEFAULT_RELEASE_NOTES = {
  de: [
    ...
    'Der LDN-Katalog mit 1787 benannten Lynds-Dunkelnebeln bleibt enthalten; LDN 1093 und LDN1093 sind suchbar.'
  ],
  en: [
    ...
    'The LDN catalog with 1,787 named Lynds dark nebulae remains included; LDN 1093 and LDN1093 are searchable.'
  ]
};
...
function versionNotesHtml(lang=language){
  const title=lang==='en'?'Changes in this version':'Änderungen in dieser Version';
  return `<h3>${title}</h3>${releaseNotesListHtml(lang)}`;
}
...
function notesFromVersionInfo(info,lang=language){
  const changes=info?.changes?.[lang]||info?.changes?.de||info?.changes?.en;
...
async function showUpdateBanner(){
  ...
  if(text){
    ...
    if(info?.version&&info.version!==APP_VERSION){
      message=language==='en'?`A new app version ${info.version} is available.`:`Eine neue App-Version ${info.version} ist verfügbar.`;
    }
    if(notes.length){

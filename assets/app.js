/* Astro Night Planner 1.0 – Produktivversion */
'use strict';
...
function normalizeProfile(p){
  ...
  out.equipment.telescopes=Array.isArray(p.equipment?.telescopes)?p.equipment.telescopes:base.equipment.telescopes;
  out.equipment.cameras=Array.isArray(p.equipment?.cameras)?p.equipment.cameras:base.equipment.cameras;
  out.equipment.mounts=Array.isArray(p.equipment?.mounts)?p.equipment.mounts:base.equipment.mounts;
  out.equipment.setups=Array.isArray(p.equipment?.setups)?p.equipment.setups:base.equipment.setups;
  ...
  if(!out.equipment.setups.length&&out.equipment.telescopes.length&&out.equipment.cameras.length)out.equipment.setups=[{id:'setup-auto',name:`${out.equipment.telescopes[0]?.name||'Teleskop'} + ${out.equipment.cameras[0]?.name||'Kamera'}`,telescopeId:out.equipment.telescopes[0]?.id||'',cameraId:out.equipment.cameras[0]?.id||'',mountId:out.equipment.mounts[0]?.id||''}];
  ...
  out.locations=(Array.isArray(p.locations)?p.locations:base.locations).map((x,locationIndex)=>{
...
function activeLocation(){const id=profile.planning?.locationId||profile.central?.defaultLocationId||profile.selectedLocationId;return profile.locations.find(x=>x.id===id)||profile.locations[0]||null}
...
function dateKeys(){const loc=activeLocation();const today=dateKeyFor(new Date(),loc?.timezone||'Europe/Berlin');return Array.from({length:8},(_,i)=>addDays(today,i))}
...
function meteoblueLocationInfo(loc){
  const geonameId=Number(loc.geonameId)||0;
...
function renderLocations(){
  ...
  return`<div class="settings-tabs subtabs">${[['locations','Standorte'],['horizon','Horizontprofile']].map(([k,n])=>`<button data-locations-subtab="${k}" class="${currentLocationsSubTab===k?'active':''}">${n}</button>`).join('')}</div><div class="card subtab-panel ${currentLocationsSubTab==='locations'?'active':''}"><div class="section-title-row"><div><h2>Gespeicherte Aufnahmeorte</h2><div class="small muted">Der Standardstandort wird hier festgelegt. In der Planung kann vorübergehend ein anderer Ort gewählt werden.</div></div><button id="addLocation">Standort hinzufügen</button></div>
    ${showAddLocationDialog?renderLocationAddDialog(country):''}
...
function bindRendered(){
  ...
  if(planningLocationSelect)planningLocationSelect.onchange=async()=>{
    ...
    const location=activeLocation();selectedDateKey=dateKeyFor(new Date(),location?.timezone||'Europe/Berlin');profile.planning.dateKey=selectedDateKey;
  ...
  const detailHorizon=document.getElementById('detailHorizonProfileSelect');if(detailHorizon)detailHorizon.onchange=async()=>{const loc=activeLocation();if(!loc)return;profile.planning.temporaryHorizonProfileId=detailHorizon.value===loc.defaultHorizonProfileId?null:detailHorizon.value;await saveProfile();render()};
  bindPlanProfiles();
  bindFraming();
...
async function fetchLocationComparison(){
  ...
  try{
    ...
    const activeLoc=activeLocation();if(!activeLoc)return;
    const activeNight=nightData(selectedDateKey,activeLoc);
...
function bindPlanProfiles(){
  ...
  if(horizon)horizon.onchange=async()=>{const loc=activeLocation();if(!loc)return;profile.planning.temporaryHorizonProfileId=horizon.value===loc.defaultHorizonProfileId?null:horizon.value;await saveProfile();render()};
}
...
function bindLocationDraft(){
  const location=()=>draft.locations.find(item=>item.id===draft.selectedLocationId)||draft.locations[0]||null,entry=()=>location()?horizonProfileFor(location(),location().selectedHorizonProfileId):null,mark=()=>{if(!location())return;syncCardinalHorizon(location(),entry()?.id);setSectionDirty('locations');syncSettingsHorizonCanvas()},set=(id,field,numeric=false)=>{const element=document.getElementById(id);if(element)element.onchange=()=>{if(!location())return;location()[field]=numeric?Number(element.value):element.value;setSectionDirty('locations')}};
...
function initCloudBaseMap(){
  ...
  const loc=activeLocation();if(!loc)return;const radius=Number(cloudMapData.radiusKm)||120,latDelta=radius/111,lonDelta=radius/(111*Math.max(.2,Math.cos(toRad(loc.latitude)))),signature=[loc.latitude,loc.longitude,radius].map(v=>Number(v).toFixed(4)).join('|');

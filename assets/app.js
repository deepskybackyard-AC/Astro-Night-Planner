/* Astro Night Planner 1.4.3 – Produktivversion */
'use strict';

const BUILD = Object.freeze(window.ANP_BUILD || {environment:'prod', appVersion:'1.4.3', release:'1.4.3', databaseName:'astro-night-planner-prod-v1', documentTitle:'Astro Night Planner 1.4.3'});
...
function updateStaticLanguageUi(){
  ...
  const updateDetailsLink=document.getElementById('updateDetailsLink');if(updateDetailsLink)updateDetailsLink.textContent=language==='en'?'All changes':'Alle Neuerungen';
  const updateBanner=document.getElementById('updateBanner');if(updateBanner&&!updateBanner.hidden)showUpdateBanner();
...
Object.assign(EN_EXACT,{
  ...
  'Lokale Quellen für alle Surveys bevorzugen':'Prefer local sources for all surveys',
  'Schaltet die Option „Lokale Quelle bevorzugen“ in allen Survey-Zeilen gemeinsam. Einzelne Surveys können danach abweichend eingestellt werden.':'Switches the “Prefer local source” option for all survey rows together. Individual surveys can then be configured differently.',
  'Relativen Pfad vom Server auswählen':'Select relative path from server',
  'Survey-Ordner auswählen':'Select survey folder',
  'Keine Survey-Ordner mit properties-Datei wurden im konfigurierten Hauptordner gefunden.':'No survey folders containing a properties file were found in the configured root folder.',
  'Gefundene Survey-Ordner':'Detected survey folders',
  'Survey-Hauptordner des Servers':'Server survey root folder',
  'Aktuell verwendet':'Currently used',
  'Diesen Ordner verwenden':'Use this folder',
  'Auswahl schließen':'Close selection',
  'Der ausgewählte relative Pfad wird für diesen Survey übernommen und die lokale Quelle aktiviert.':'The selected relative path will be used for this survey and the local source will be enabled.',
  'Der Local Survey Server ist nicht erreichbar. Starte ihn zuerst und versuche es erneut.':'The Local Survey Server is not reachable. Start it first and try again.',
  'Survey-Daten herunterladen und verwalten':'Download and manage survey data',
  'Öffnet die Downloadverwaltung des laufenden Local Survey Servers. Dort können alle in der App eingebauten Surveys mit geprüften Standardadressen sowie weitere kompatible HiPS-Surveys geladen, pausiert, fortgesetzt, abgebrochen und geprüft werden. Quelle und Zielpfad lassen sich pro Survey speichern.':'Opens the download manager of the running Local Survey Server. All surveys built into the app are available with verified default service URLs, and additional compatible HiPS surveys can be downloaded, paused, resumed, cancelled and validated. Source and target path can be saved per survey.',
  ...
  'Windows-Serverprogramm herunterladen':'Download Windows server helper',
  'Linux/macOS-Server herunterladen':'Download Linux/macOS server',
  'Python-Fallback herunterladen':'Download Python fallback',
  'Windows: Browseroberfläche mit automatischem Start, Tray und Autostart. Linux/macOS: natives Serverprogramm mit Browseroberfläche; kein Wine erforderlich.':'Windows: browser interface with automatic opening, tray and autostart. Linux/macOS: native server program with browser interface; Wine is not required.',
  'Unter Linux und macOS muss der Server manuell gestartet oder per systemd/LaunchAgent automatisch gestartet werden. Die App kann lokale Programme nicht direkt starten.':'On Linux and macOS, the server must be started manually or automatically via systemd/LaunchAgent. The app cannot start local programs directly.',
  'Das Windows-Hilfsprogramm ist nur für Windows vorgesehen. Für Linux und macOS verwende das separate native Serverpaket.':'The Windows helper is intended for Windows only. For Linux and macOS, use the separate native server package.',
  ...
  'Der optionale Local Survey Server 1.1 (Build 1.1.5) kann lokale HiPS-Dateien ohne Python bereitstellen und alle in der App eingebauten sowie benutzerdefinierte kompatible HiPS-Surveys herunterladen.':'The optional Local Survey Server 1.1 (Build 1.1.5) can serve local HiPS files without Python and download all surveys built into the app as well as compatible custom HiPS surveys.',
  'Local Survey Server öffnen':'Open Local Survey Server',
  'Local Survey Server nicht erreichbar. Starte ANP-Local-Survey-Server.exe oder aktiviere den Autostart.':'Local Survey Server not reachable. Start ANP-Local-Survey-Server.exe or enable autostart.',
  ...
  'Betriebssystem-Anleitungen öffnen':'Open operating-system guides',
  'Windows-Anleitung':'Windows guide',
  'Linux/macOS-Anleitung':'Linux/macOS guide',
  'Linux-Autostart':'Linux autostart',
  'macOS-Autostart':'macOS autostart',
  'Python-Fallback-Anleitung':'Python fallback guide',
  'Ausführliche HTML-Anleitungen':'Detailed HTML guides',
  'Die ausführlichen Schritt-für-Schritt-Anleitungen stehen als HTML-Seiten bereit.':'Detailed step-by-step instructions are available as HTML pages.',
...
function resolvedAladinSurvey(item,source=profile){const cfg=normalizeLocalSurveySettings(source?.central?.localSurveys, normalizeAladinSurveys(source?.central?.aladinSurveys)),entry=item?cfg.surveys[item.id]:null;if(cfg.enabled&&entry?.enabled&&entry.path)return localSurveyUrlFor(item,source);return item?.hipsId||''}
...
function releaseNotesListHtml(lang=language){
  ...
  const critical=BUILD.criticalUpdate?.[lang]||BUILD.criticalUpdate?.de||'';
  const details=BUILD.detailsUrl?.[lang]||BUILD.detailsUrl?.de||'';
  const criticalHtml=critical?`<div class="release-critical"><strong>${lang==='en'?'Important for this update':'Wichtig für dieses Update'}:</strong> ${esc(critical)}</div>`:'';
  const linkHtml=details?`<p><a class="button" href="${esc(details)}" target="_blank" rel="noopener">${lang==='en'?'Open complete version history':'Vollständige Neuerungsübersicht öffnen'}</a></p>`:'';
  return `${criticalHtml}${notes.length?`<ul>${notes.map(item=>`<li>${esc(item)}</li>`).join('')}</ul>`:''}${linkHtml}`;
}
function versionNotesHtml(lang=language){
  ...
  const preferred=['version143','version142','version141','version140','version130','version120','version110'];
...
async function showUpdateBanner(){
  const banner=document.getElementById('updateBanner'),text=document.getElementById('updateBannerText'),detailsLink=document.getElementById('updateDetailsLink');
  if(!banner)return;
  let info=null;
  try{info=await loadOnlineVersionInfo()}catch(error){}
  const critical=info?.criticalUpdate?.[language]||info?.criticalUpdate?.de||BUILD.criticalUpdate?.[language]||BUILD.criticalUpdate?.de||'';
  if(text){
    ...
    text.innerHTML=`<span>${esc(message)}</span>${critical?`<strong class="update-critical">⚠ ${esc(critical)}</strong>`:''}`;
  }
  if(detailsLink){
    const url=info?.detailsUrl?.[language]||info?.detailsUrl?.de||BUILD.detailsUrl?.[language]||BUILD.detailsUrl?.de||'';
    detailsLink.hidden=!url;
    if(url){detailsLink.href=url;detailsLink.textContent=language==='en'?'All changes':'Alle Neuerungen'}
  }
  banner.hidden=false;
}
...
function renderFraming(o,windowRange,loc){
  ...
  const localSurveyCandidate=localSurveyCfg.enabled&&localSurveyEntry?.enabled&&localSurveyEntry.path?localSurveyUrlFor(surveyItem,profile):'';
...
function renderAladinSurveySettings(c){
  ...
  const localEntries=surveys.map(item=>local.surveys[item.id]||{enabled:false,path:defaultLocalSurveyPath(item),lastStatus:'unchecked'});
  const allLocalPreferred=localEntries.length>0&&localEntries.every(entry=>entry.enabled);
  const someLocalPreferred=localEntries.some(entry=>entry.enabled);
  const localRows=surveys.map(item=>{const entry=local.surveys[item.id]||{enabled:false,path:defaultLocalSurveyPath(item),lastStatus:'unchecked'};const full=joinSurveyUrl(local.baseUrl,entry.path),statusText=optionText(entry.lastStatus==='ok'?'Lokale Quelle erreichbar.':entry.lastStatus==='failed'?'Lokale Quelle nicht erreichbar.':'Nicht geprüft');return `<details class="local-survey-row" data-local-survey-details="${esc(item.id)}" ${openLocalSurveyId===item.id?'open':''}><summary><strong>${esc(item.name)}</strong><span class="small muted">${esc(optionLabel(item.category||''))}</span><code class="local-survey-path">${esc(entry.path)}</code><span class="small muted">${statusText}</span></summary><div class="grid two" style="margin-top:10px"><label class="chip"><input data-local-survey="${esc(item.id)}" data-field="enabled" type="checkbox" ${entry.enabled?'checked':''}>${optionText('Lokale Quelle bevorzugen')}</label><label>${optionText('Lokaler relativer Pfad')}<span class="input-button-row"><input data-local-survey="${esc(item.id)}" data-field="path" value="${esc(entry.path)}"><button type="button" data-pick-local-survey="${esc(item.id)}">${optionText('Relativen Pfad vom Server auswählen')}</button></span></label><label>${optionText('Online-Quelle')}<input value="${esc(item.hipsId)}" readonly></label><label>${optionText('Vollständige lokale URL')}<input value="${esc(full)}" readonly></label></div><div class="inline" style="margin-top:10px"><button type="button" data-test-local-survey="${esc(item.id)}">${optionText('Lokale Survey-Quelle prüfen')}</button><span class="small muted">${optionText('Status lokale Quelle')}: ${statusText}</span></div></details>`}).join('');
  return `<details class="aladin-survey-settings" open><summary><span class="disclosure-arrow" aria-hidden="true"></span><strong>${optionText('Aladin - Survey')}</strong><span class="small muted">${optionText('Weitere Surveys können manuell per HiPS-ID ergänzt werden. Monochrome Surveys werden nativ angezeigt.')}</span></summary><div class="settings-tabs subtabs mini-tabs" style="margin-top:14px">${[['surveys','Surveys'],['local','Lokale Survey-Quellen']].map(([k,n])=>`<button type="button" data-aladin-survey-subtab="${k}" class="${currentAladinSurveySubTab===k?'active':''}">${optionText(n)}</button>`).join('')}</div><div class="subtab-panel ${currentAladinSurveySubTab==='surveys'?'active':''}"><div class="section-title-row" style="margin-top:12px"><div><div class="small muted">${language==='en'?'Activate surveys in the Aladin selection, rename them or add custom HiPS IDs. Use the Local source button for optional offline/local HiPS configuration.':'Surveys in der Aladin-Auswahl aktivieren, umbenennen oder eigene HiPS-IDs ergänzen. Über „Lokale Quelle“ wird optional die lokale/Offline-Quelle konfiguriert.'}</div></div><button id="addAladinSurvey" type="button">${optionText('Survey hinzufügen')}</button></div><div class="aladin-survey-list">${surveyRows}</div></div><div class="subtab-panel ${currentAladinSurveySubTab==='local'?'active':''}"><div class="section-title-row" style="margin-top:12px"><div><h4>${optionText('Lokale Survey-Quellen')}</h4><div class="small muted">${optionText('Ein normaler Dateipfad kann vom Browser nicht direkt gelesen werden. Verwende eine lokale HTTP-Adresse, z. B. http://127.0.0.1:8765/.')}</div></div><button type="button" data-open-help-section="help-local-surveys">${optionText('Hilfe zu lokalen Surveys öffnen')}</button></div><div class="grid two" style="margin-top:12px"><label class="chip"><input id="localSurveysEnabled" type="checkbox" ${local.enabled?'checked':''}>${optionText('Lokale Survey-Nutzung aktivieren')}</label><label>${optionText('Lokale Survey-Basis-URL')}<input id="localSurveyBaseUrl" value="${esc(local.baseUrl)}" placeholder="http://127.0.0.1:8765/"></label><label class="chip"><input id="localSurveyPreferLocal" type="checkbox" ${allLocalPreferred?'checked':''} data-mixed="${someLocalPreferred&&!allLocalPreferred?'true':'false'}">${optionText('Lokale Quellen für alle Surveys bevorzugen')}</label><label class="chip"><input id="localSurveyFallbackOnline" type="checkbox" ${local.fallbackOnline?'checked':''}>${optionText('Online-Quelle verwenden, wenn lokale Quelle nicht erreichbar ist')}</label></div><div class="small muted" style="margin-top:8px">${optionText('Schaltet die Option „Lokale Quelle bevorzugen“ in allen Survey-Zeilen gemeinsam. Einzelne Surveys können danach abweichend eingestellt werden.')}</div><div class="inline" style="margin:12px 0"><button type="button" id="testLocalSurveyBase">${optionText('Lokale Survey-Quelle prüfen')}</button><button type="button" id="openLocalSurveyServerUi">${optionText('Local Survey Server öffnen')}</button><button type="button" id="openLocalSurveyDownloads">${optionText('Survey-Daten herunterladen und verwalten')}</button><a class="btn" href="tools/ANP-Local-Survey-Server-1.1.zip" download>${optionText('Windows-Serverprogramm herunterladen')}</a><a class="btn" href="tools/ANP-Local-Survey-Server-Linux-macOS-1.1.zip" download>${optionText('Linux/macOS-Server herunterladen')}</a><a class="btn" href="tools/ANP-Local-Survey-Server-Python-1.0.zip" download>${optionText('Python-Fallback herunterladen')}</a><span class="small muted">${optionText('Windows: Browseroberfläche mit automatischem Start, Tray und Autostart. Linux/macOS: natives Serverprogramm mit Browseroberfläche; kein Wine erforderlich.')}</span></div><div class="inline small" style="margin:0 0 12px 0"><strong>${optionText('Ausführliche HTML-Anleitungen')}:</strong><a class="btn" href="docs/local-survey/${language==='en'?'index-en.html':'index-de.html'}" target="_blank" rel="noopener">${optionText('Betriebssystem-Anleitungen öffnen')}</a><a class="btn" href="docs/local-survey/${language==='en'?'windows-en.html':'windows-de.html'}" target="_blank" rel="noopener">${optionText('Windows-Anleitung')}</a><a class="btn" href="docs/local-survey/${language==='en'?'linux-macos-en.html':'linux-macos-de.html'}" target="_blank" rel="noopener">${optionText('Linux/macOS-Anleitung')}</a><a class="btn" href="docs/local-survey/${language==='en'?'downloads-en.html':'downloads-de.html'}" target="_blank" rel="noopener">${optionText('Survey-Daten herunterladen und verwalten')}</a><a class="btn" href="docs/local-survey/${language==='en'?'python-en.html':'python-de.html'}" target="_blank" rel="noopener">${optionText('Python-Fallback-Anleitung')}</a></div><div class="callout small" style="margin:0 0 12px 0">${optionText('Das Windows-Hilfsprogramm ist nur für Windows vorgesehen. Für Linux und macOS verwende das separate native Serverpaket.')} ${optionText('Unter Linux und macOS muss der Server manuell gestartet oder per systemd/LaunchAgent automatisch gestartet werden. Die App kann lokale Programme nicht direkt starten.')} ${optionText('Die ausführlichen Schritt-für-Schritt-Anleitungen stehen als HTML-Seiten bereit.')}</div><div class="local-survey-list">${localRows}</div></div></details>`;
}
...
function renderInfo(){
  ...
  <div class="card help-article subtab-panel ${currentInfoSubTab==='help'?'active':''}"><div class="section-title-row"><div><h2>${esc(tx('helpTitle'))}</h2><div class="small muted">${esc(tx('helpSub'))}</div><div class="help-language-note">${esc(tx('helpNote'))}</div></div><div class="data-actions"><button type="button" data-open-help-section="help-storage">${esc(language==='en'?'Open integrated help':'Integrierte Hilfe öffnen')}</button><a class="button primary" href="${docLink('pdf')}" target="_blank" rel="noopener">${esc(tx('helpPdf'))}</a></div></div>
    ...
    <section id="help-news"><h3>${language==='en'?'What’s new':'Neuerungen'}</h3><p>${language==='en'?'Changes are listed by version. The latest release is shown first, followed by earlier update steps.':'Die Neuerungen werden versionsweise angezeigt. Die aktuelle Version steht zuerst; danach folgen die vorherigen Versionsschritte.'}</p><p><a class="button" href="docs/${language==='en'?'NEUERUNGEN_EN.html':'NEUERUNGEN_DE.html'}" target="_blank" rel="noopener">${language==='en'?'Open complete version history':'Vollständige Neuerungsübersicht öffnen'}</a></p>${versionNotesHtml(language)}</section>
    ...
    ${language==='en'?`<section id="help-local-surveys"><h3>Local surveys and Local Survey Server</h3><p>Local surveys are optional local copies of HiPS sky maps. They are not used through file paths, but through a local HTTP address such as <code>http://127.0.0.1:8765/</code>. Aladin expects <code>properties</code>, <code>Norder...</code>, <code>Dir...</code>, <code>Npix...</code> and <code>Allsky...</code> through URLs.</p><div class="callout"><h4>Detailed HTML guides</h4><p>The README instructions are available as readable HTML pages:</p><p><a class="button" href="docs/local-survey/index-en.html" target="_blank" rel="noopener">Operating-system overview</a> <a class="button" href="docs/local-survey/windows-en.html" target="_blank" rel="noopener">Windows guide</a> <a class="button" href="docs/local-survey/linux-macos-en.html" target="_blank" rel="noopener">Linux/macOS guide</a> <a class="button" href="docs/local-survey/linux-autostart-en.html" target="_blank" rel="noopener">Linux autostart</a> <a class="button" href="docs/local-survey/macos-autostart-en.html" target="_blank" rel="noopener">macOS autostart</a> <a class="button" href="docs/local-survey/downloads-en.html" target="_blank" rel="noopener">Survey downloads</a> <a class="button" href="docs/local-survey/python-en.html" target="_blank" rel="noopener">Python fallback</a></p></div><h4>Common principle</h4><p>The Astro Night Planner settings are the same on Windows, Linux and macOS: base URL <code>http://127.0.0.1:8765/</code> and one relative path per survey, for example <code>nsns/ohs8/</code>.</p><h4>Which helper should be used?</h4><ul><li><strong>Windows:</strong> use the Windows helper with an automatically opened browser interface, tray menu, autostart and exit function.</li><li><strong>Linux/macOS:</strong> use the native Linux/macOS package with browser interface. Wine is not required.</li><li><strong>Python:</strong> use the Python package as a transparent fallback or expert option.</li></ul><h4>Can the web app start the server?</h4><p>No. A web app cannot directly start local programs. The <strong>Open Local Survey Server</strong> button can only open the local configuration interface when the helper is already running. If the server is not reachable, start the helper manually or enable autostart.</p><h4>Folder selection and survey downloads</h4><p>The Local Survey Server 1.1 (Build 1.1.5) provides operating-system folder dialogs for the survey root and download target. In the Planner, the button next to a relative path lists folders containing a <code>properties</code> file that the running server has detected. The <strong>Download and manage survey data</strong> button opens the server download manager. All surveys built into the app are preconfigured with verified default service URLs. Additional compatible HiPS surveys can be downloaded from a custom source URL and relative target path, paused, resumed, cancelled and validated. Source and target path can be saved per survey. The downloader verifies <code>properties</code> and <code>Moc.fits</code> and derives the required tiles from the MOC coverage map; a directory listing is not required.</p><h4>Interrupted connections and automatic retry</h4><p>Build 1.1.5 retries temporary network errors, timeouts and HTTP 429/5xx automatically with increasing delays. A single failed tile no longer stops the complete download. Downloads from <code>www.simg.de</code> use two parallel connections; other sources use four by default. After the main pass, remaining files are retried automatically. Existing <code>.part</code> files are continued.</p><h4>Windows tray reliability</h4><p>The tray controller runs on a dedicated Windows UI thread, restores the tray icon after Explorer restarts and supports current tray notification events. <strong>Exit</strong> includes a safety fallback so the program can be replaced without Task Manager.</p><h4>Global and per-survey preference</h4><p><strong>Prefer local sources for all surveys</strong> is a bulk switch: it checks or unchecks the <strong>Prefer local source</strong> option in every survey row. Individual survey rows can then be changed separately.</p><h4>Fallback and diagnostics</h4><p>The Planner uses the local source only when local usage is enabled and the source is reachable. Online fallback is used only when it is enabled. If fallback is disabled and the local source cannot be loaded, an error message is shown instead of silently switching to DSS or another default background.</p></section>`:`<section id="help-local-surveys"><h3>Lokale Surveys und Local Survey Server</h3><p>Lokale Surveys sind optionale lokale Kopien von HiPS-Himmelskarten. Sie werden nicht über Dateipfade, sondern über eine lokale HTTP-Adresse wie <code>http://127.0.0.1:8765/</code> genutzt. Aladin erwartet <code>properties</code>, <code>Norder...</code>, <code>Dir...</code>, <code>Npix...</code> und <code>Allsky...</code> als URLs.</p><div class="callout"><h4>Ausführliche HTML-Anleitungen</h4><p>Die README-Anleitungen stehen zusätzlich als gut lesbare HTML-Seiten bereit:</p><p><a class="button" href="docs/local-survey/index-de.html" target="_blank" rel="noopener">Betriebssystem-Übersicht</a> <a class="button" href="docs/local-survey/windows-de.html" target="_blank" rel="noopener">Windows-Anleitung</a> <a class="button" href="docs/local-survey/linux-macos-de.html" target="_blank" rel="noopener">Linux/macOS-Anleitung</a> <a class="button" href="docs/local-survey/linux-autostart-de.html" target="_blank" rel="noopener">Linux-Autostart</a> <a class="button" href="docs/local-survey/macos-autostart-de.html" target="_blank" rel="noopener">macOS-Autostart</a> <a class="button" href="docs/local-survey/downloads-de.html" target="_blank" rel="noopener">Survey-Downloads</a> <a class="button" href="docs/local-survey/python-de.html" target="_blank" rel="noopener">Python-Fallback</a></p></div><h4>Gemeinsames Prinzip</h4><p>Die Konfiguration im Astro Night Planner ist unter Windows, Linux und macOS gleich: Basis-URL <code>http://127.0.0.1:8765/</code> und pro Survey ein relativer Pfad, zum Beispiel <code>nsns/ohs8/</code>.</p><h4>Welche Variante soll verwendet werden?</h4><ul><li><strong>Windows:</strong> Windows-Hilfsprogramm mit automatisch geöffneter Browseroberfläche, Tray-Menü, Autostart und Beenden-Funktion.</li><li><strong>Linux/macOS:</strong> natives Linux/macOS-Paket mit Browseroberfläche. Wine wird nicht benötigt.</li><li><strong>Python:</strong> Python-Paket als transparenter Fallback oder Expertenlösung.</li></ul><h4>Kann die Web-App den Server starten?</h4><p>Nein. Eine Web-App kann lokale Programme nicht direkt starten. Der Button <strong>Local Survey Server öffnen</strong> kann nur die lokale Konfigurationsoberfläche öffnen, wenn der Server bereits läuft. Wenn er nicht erreichbar ist, muss das passende Hilfsprogramm manuell gestartet oder per Autostart aktiviert werden.</p><h4>Ordnerauswahl und Survey-Downloads</h4><p>Der Local Survey Server 1.1 (Build 1.1.5) bietet Betriebssystem-Ordnerdialoge für den Survey-Hauptordner und das Downloadziel. Im Planner listet der Button neben einem relativen Pfad alle vom laufenden Server gefundenen Ordner mit <code>properties</code>-Datei auf. Der Button <strong>Survey-Daten herunterladen und verwalten</strong> öffnet die Downloadverwaltung. Alle in der App eingebauten Surveys sind mit Standard-Quelladressen vorbelegt. Zusätzlich können benutzerdefinierte HiPS-Surveys über eine eigene Quelladresse und einen relativen Zielpfad geladen, pausiert, fortgesetzt, abgebrochen und geprüft werden. Der Downloader prüft <code>properties</code> und <code>Moc.fits</code> und ermittelt die benötigten Kacheln aus der MOC-Abdeckung; ein Verzeichnislisting ist nicht erforderlich.</p><h4>Verbindungsabbrüche und automatische Wiederholung</h4><p>Build 1.1.5 wiederholt temporäre Netzwerkfehler, Timeouts sowie HTTP 429/5xx automatisch mit zunehmender Wartezeit. Eine einzelne fehlerhafte Kachel beendet den Gesamtdownload nicht mehr. Downloads von <code>www.simg.de</code> verwenden zwei parallele Verbindungen; andere Quellen standardmäßig vier. Nach dem Hauptlauf werden verbleibende Dateien automatisch erneut versucht. Vorhandene <code>.part</code>-Dateien werden fortgesetzt.</p><h4>Stabilität des Windows-Trays</h4><p>Der Tray-Controller läuft auf einem festen Windows-UI-Thread, stellt das Symbol nach einem Explorer-Neustart wieder her und unterstützt aktuelle Tray-Ereignisse. <strong>Beenden</strong> besitzt eine Sicherheitsabschaltung, damit das Programm ohne Task-Manager ausgetauscht werden kann.</p><h4>Globale und einzelne Bevorzugung</h4><p><strong>Lokale Quellen für alle Surveys bevorzugen</strong> ist jetzt ein Sammelschalter: Er setzt oder entfernt das Häkchen <strong>Lokale Quelle bevorzugen</strong> in allen Survey-Zeilen. Einzelne Survey-Zeilen können danach abweichend eingestellt werden.</p><h4>Fallback und Diagnose</h4><p>Der Planner verwendet lokale Quellen nur, wenn lokale Nutzung aktiviert und die Quelle erreichbar ist. Der Online-Fallback wird nur verwendet, wenn er aktiviert ist. Ist der Fallback deaktiviert und die lokale Quelle kann nicht geladen werden, erscheint eine Fehlermeldung statt eines stillen Wechsels auf DSS oder einen anderen Standardhintergrund.</p></section>`}    <section id="help-horizon"><h3>Standorte und interaktive Horizontprofile</h3><p>Für jeden Standort können mehrere Horizontprofile angelegt werden. Die Standortsuche zeigt eine Trefferliste mit Ort, Region, Land, Postleitzahl und Koordinaten; ein bevorzugtes Land verhindert, dass eine deutsche PLZ ungeprüft einem weltweiten Ersttreffer zugeordnet wird. Die Zeitzone wird automatisch ermittelt und kann nur aus gültigen IANA-Zeitzonen gewählt werden.</p><p>Ein Horizontprofil wird als Standard festgelegt; in Planung und Detailansicht kann vorübergehend ein anderes gewählt werden. Bearbeite die Linie direkt mit Maus oder Finger. Hindernisse gehören zum jeweiligen Profil.</p></section>
...
function bindCentralDraft(){
  ...
  const localPrefer=document.getElementById('localSurveyPreferLocal');if(localPrefer){localPrefer.indeterminate=localPrefer.dataset.mixed==='true';localPrefer.onchange=()=>{const cfg=ensureLocal();cfg.preferLocal=localPrefer.checked;Object.values(cfg.surveys||{}).forEach(entry=>{entry.enabled=localPrefer.checked;entry.lastStatus='unchecked'});setSectionDirty('display');render()}};
  ...
  async function locateLocalSurveyServerUi(){
    const ports=[8776,8777,8778,8779,8780,8781,8782,8783,8784,8785,8786];
    const tryUrl=async(url)=>{const controller=('AbortController' in window)?new AbortController():null;const timer=controller?setTimeout(()=>controller.abort(),700):null;try{const response=await fetch(url+'api/status',{method:'GET',cache:'no-store',signal:controller?.signal});if(!response.ok)return null;await response.json();return url}catch(error){return null}finally{if(timer)clearTimeout(timer)}};
    for(const port of ports){const found=await tryUrl(`http://127.0.0.1:${port}/`);if(found)return found}
    return null;
  }
  async function openLocalSurveyServerUi(hash=''){
    const popup=window.open('about:blank','_blank');
    const url=await locateLocalSurveyServerUi();
    if(url){if(popup)popup.location.href=url+(hash||'');else window.open(url+(hash||''),'_blank');return url}
    if(popup)popup.close();
    alert(language==='en'
      ? 'Local Survey Server not reachable. Start the server helper for your operating system or enable autostart. The app cannot start local programs directly; it can only open the local configuration interface when the helper is already running.'
      : 'Local Survey Server nicht erreichbar. Starte das Server-Hilfsprogramm für dein Betriebssystem oder aktiviere den Autostart. Die App kann lokale Programme nicht direkt starten; sie kann nur die lokale Konfigurationsoberfläche öffnen, wenn das Hilfsprogramm bereits läuft.');
    return null;
  }
  function showLocalSurveyFolderDialog(id,folders,current,root){
    document.getElementById('localSurveyFolderDialog')?.remove();
    const dialog=document.createElement('dialog');
    dialog.id='localSurveyFolderDialog';
    dialog.className='target-search-dialog';
    const rows=folders.map(folder=>`<button type="button" class="target-search-result${folder===current?' active':''}" data-folder="${esc(folder)}"><strong>${esc(folder)}</strong><span>${folder===current?optionText('Aktuell verwendet'):optionText('Diesen Ordner verwenden')}</span></button>`).join('');
    dialog.innerHTML=`<form method="dialog" class="legal-dialog-shell target-search-shell"><div class="legal-dialog-header"><h2>${optionText('Gefundene Survey-Ordner')}</h2><button type="submit" class="legal-dialog-close" aria-label="${optionText('Auswahl schließen')}">✕</button></div><div class="legal-dialog-content"><p>${optionText('Der ausgewählte relative Pfad wird für diesen Survey übernommen und die lokale Quelle aktiviert.')}</p><div class="soft"><strong>${optionText('Survey-Hauptordner des Servers')}:</strong><br><code>${esc(root||'–')}</code></div><div class="target-search-results">${rows}</div></div><div class="legal-dialog-actions"><button type="submit" class="secondary">${optionText('Auswahl schließen')}</button></div></form>`;
    dialog.querySelectorAll('[data-folder]').forEach(button=>button.addEventListener('click',()=>{
      const folder=button.dataset.folder||'';
      if(!folder)return;
      const cfg=ensureLocal(),entry=cfg.surveys[id]||(cfg.surveys[id]={enabled:false,path:'',lastStatus:'unchecked'});
      entry.path=folder;entry.enabled=true;entry.lastStatus='unchecked';setSectionDirty('display');dialog.close();dialog.remove();render();
    }));
    dialog.addEventListener('close',()=>dialog.remove(),{once:true});
    document.body.appendChild(dialog);dialog.showModal?.();
  }
  async function selectLocalSurveyFolder(id){
    const url=await locateLocalSurveyServerUi();
    if(!url){alert(optionText('Der Local Survey Server ist nicht erreichbar. Starte ihn zuerst und versuche es erneut.'));return}
    try{
      const response=await fetch(url+'api/folders',{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);
      const data=await response.json(),folders=Array.isArray(data.folders)?data.folders:[];
      if(!folders.length){alert(optionText('Keine Survey-Ordner mit properties-Datei wurden im konfigurierten Hauptordner gefunden.'));return}
      const current=ensureLocal().surveys[id]?.path||'';
      showLocalSurveyFolderDialog(id,folders,current,data.root||'');
    }catch(error){alert(`${optionText('Fehler')}: ${error.message||error}`)}
  }

  document.getElementById('openLocalSurveyServerUi')?.addEventListener('click',()=>openLocalSurveyServerUi());
  document.getElementById('openLocalSurveyDownloads')?.addEventListener('click',()=>openLocalSurveyServerUi('#downloads'));
  document.querySelectorAll('[data-pick-local-survey]').forEach(button=>button.onclick=()=>selectLocalSurveyFolder(button.dataset.pickLocalSurvey));

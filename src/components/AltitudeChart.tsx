...
function buildBands(night: NightWindow, start: Date, end: Date): TwilightBand[] {
  const raw: Array<[Date | undefined, Date | undefined, TwilightBand['kind'], string]> = [
    [start, night.civilDusk, 'civil', 'bürgerlich'],
    [night.civilDusk, night.nauticalDusk, 'nautical', 'nautisch'],
    [night.nauticalDusk, night.astronomicalDusk, 'astronomical', 'astronomisch'],
    [night.astronomicalDusk, night.astronomicalDawn, 'dark', 'Nacht'],
    [night.astronomicalDawn, night.nauticalDawn, 'astronomical', 'astronomisch'],
    [night.nauticalDawn, night.civilDawn, 'nautical', 'nautisch'],
    [night.civilDawn, end, 'civil', 'bürgerlich'],
  ];
  return raw
    .filter((item): item is [Date, Date, TwilightBand['kind'], string] => Boolean(item[0] && item[1] && item[1]!.getTime() > item[0]!.getTime()))
    .map(([bandStart, bandEnd, kind, label]) => ({
...
export default function AltitudeChart({ object, night, location, timezone, minAltitude, selectedTime, onSelectedTimeChange }: Props) {
  const data = useMemo(() => {
  ...
  return (
    <section className="altitude-section" aria-label={`Höhenverlauf von ${object.name}`}>
      <div className="altitude-heading">
        <div>
          <span className="eyebrow">Objekthöhe über die Nacht</span>
          <h3>Höhenkurve</h3>
        </div>
      ...
      <div className="altitude-chart-scroll">
        ...
        >
          ...
          {data.bands.map((band, index) => {
            ...
            return <g key={`${band.kind}-${index}`}>
              <rect className={`twilight-band ${band.kind}`} x={x} y={MARGIN.top} width={width} height={PLOT_HEIGHT} />
              {width > 70 && <text className="twilight-label" x={x + width / 2} y={MARGIN.top + 16} textAnchor="middle">{band.label}</text>}
            </g>;
          })}
          ...
          <circle className="max-altitude-point" cx={xForTime(data.maxPoint.time)} cy={yForAltitude(data.maxPoint.altitude)} r="6" />
          ...
          <circle className="selected-altitude-point" cx={xForTime(selected.time)} cy={yForAltitude(selected.altitude)} r="5" />
        </svg>
      </div>
